          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2 text-slate-800 font-semibold px-2">
                ...
                <span>{isTH ? "ตารางข้อมูลบัญชีผู้ดูแล" : "Admin Account Records"}</span>
              </div>
              <Link href="/admin/new" className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-slate-800 transition-colors">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-left py-4 px-3 font-semibold text-slate-700">{isTH ? "ประเภทบัญชี" : "Account Type"}</th>
                ...
                <tbody>
                  {mockAdmins.map((row) => (
                    <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-4 px-3 text-slate-600">{row.accountType}</td>
                      ...
                      <td className="py-4 px-3 text-center">
                        <Link href={`/admin/${row.id}/edit`} className="text-slate-400 hover:text-slate-700 transition-colors inline-block">
