...
const mockVisits = [
  { id: 1, name: "วางบิล" },
  { id: 2, name: "เสนอโปรเจค" },
]

const chartData = [
  { name: "วางบิล", value: 90 },
  { name: "เสนอโปรเจค", value: 65 },
]
...
export default function VisitPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white font-sans">
      <Headbar title={isTH ? "ภารกิจเยี่ยมชม" : "Visit Mission"} showFactoryFilter={false} showDateFilter={false} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div>
                <div className="flex items-center gap-2 mb-6 text-slate-800 font-semibold">
                  ...
                  <span>{isTH ? "กราฟภารกิจเยี่ยมชม" : "Visit Mission Chart"}</span>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-2 text-slate-800 font-semibold">
                    <Clock className="w-5 h-5" />
                    <span>{isTH ? "ตารางภารกิจเยี่ยมชม" : "Visit Mission List"}</span>
                  </div>
                  <Link href="/visit/new" className="flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-full text-sm font-medium hover:bg-slate-800 transition-colors">
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-200">
                        ...
                        <th className="text-center py-4 px-4 font-semibold text-slate-700">{isTH ? "ภารกิจเยี่ยมชม" : "Visit Mission"}</th>
                      ...
                      {mockVisits.map((row) => (
                        <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-4 px-4 text-center">
                            <Link href={`/visit/${row.id}/edit`} className="text-slate-400 hover:text-slate-700 transition-colors inline-block">
